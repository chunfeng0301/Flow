...
def train(args):
    ...
    with fluid.dygraph.guard(place):
        ...
        """
        train_model = TSN18.TSNResNet('TSN',train_config['MODEL']['num_layers'],
                                     train_config['MODEL']['num_classes'], 
                                     train_config['MODEL']['seg_num'],0.00002)
        """
        train_model = baseline_2d_resnets_pp.ResNet50Flow('ResNet50Flow',train_config['MODEL']['num_layers'],
                                     train_config['MODEL']['num_classes'], 
                                     train_config['MODEL']['seg_num'])

        #根据自己定义的网络，声明train_model
        #opt=fluid.optimizer.AdamOptimizer(learning_rate=0.01, beta1=0.9, beta2=0.999, epsilon=1e-08, parameter_list=train_model.parameters())
        opt=fluid.optimizer.MomentumOptimizer(learning_rate=0.005, momentum=0.9, parameter_list=train_model.parameters())
        #opt = fluid.optimizer.Momentum(0.0005, 0.9, parameter_list=train_model.parameters())
        ...
        for i in range(epochs):
            ...
            if i % 1 == 0:
                ...
                with fluid.dygraph.guard():
                    fluid.dygraph.save_dygraph(train_model.state_dict(), args.save_dir + '/tsn_model'+str(i)+str(acc.numpy()))
                    weights = args.save_dir + '/tsn_model'+str(i)+str(acc.numpy())
                    acceval= eval2(weights,16)
                    fluid.dygraph.save_dygraph(train_model.state_dict(), args.save_dir + '/tsn_model'+str(acceval))
